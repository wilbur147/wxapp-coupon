...
const addRess = require('./movie-address.js')
const app = getApp()
...
Page({
  ...
  selectItem(e) {
    ...
    if (item === 1 && !this.data.mostExpectedList.length) {
      ...
      request({
        api: '/program/movie/mostExpected?limit=10&offset=0&token='
      }).then(([res]) => {
        this.setData({
  ...
  async ReachBottom(list, ids, complete, item) {
    ...
    const [res,err] = await request({
      api:`/program/movie/moreComingList?movieIds=${query}`
    })
    if(!err){
  ...
  async lower() {
    ...
    const [res, err] = await request({
      api: "/program/movie/mostExpected",
      data: {
        limit: 10,
        offset: length,
        token:''
